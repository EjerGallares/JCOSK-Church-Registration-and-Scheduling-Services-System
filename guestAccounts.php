...
<section class="home-section">
    <nav>
      <div class="sidebar-button">
        ...
        <span class="dashboard">Guest Accounts</span>
      </div>
...
<?php 
  ...
  or die(mysql_error($mysqli));
  $result = $mysqli->query ('SELECT * FROM tbl_guestinfo ') or die($mysqli->error);
?>
   <form action="process.php" method="Post">
  <table id="exampleReg" class="display" style="width:100%">
    ...
    while ($row = $result-> fetch_assoc()):?>
     <tr>
              ...
              <td><?php echo $row['guest_Fname']; ?></td>
              <td><?php echo $row['guest_Add']; ?></td>
              <td><?php echo $row['guest_Con']; ?></td>
              <td><?php echo $row['guest_Email']; ?></td>
              <td><?php echo $row['guest_date']; ?></td>
              <td style="width:10px; text-align: center">
               <input type="checkbox" name="delcheck[]" value="<?= $row['guest_ID']; ?>">
            </td> 
