  ...
  <section class="home-section">
    <nav>
      <div class="sidebar-button">
        ...
        <span class="dashboard">Manage event</span>
      </div>
   <p class = "greet">
        <script>
          let date = new Date();
...
<br>
  ...
  or die(mysql_error($mysqli));
  $result = $mysqli->query ('SELECT * FROM tbl_events  ') or die($mysqli->error);
?>
  ...
  <table id="exampleReg" class="display" style="width:100%">
    ...
    while ($row = $result-> fetch_assoc()):?>
     <tr>
              ...
              <td><?php echo $row['event_officiator']; ?></td>
              <td style="width:10px; text-align: center">
               <input type="checkbox" name="delcheck[]" value="<?= $row['event_ID']; ?>">
            </td> 
